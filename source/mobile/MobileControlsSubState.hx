...
class MobileControlsSubState extends FlxSubState
{
	...
	private var dodgePosition:FlxText;
	...
	private var isExtraButton:Bool = (Init.trueSettings.get('Mechanics Type') == "Button");
	...
	override function create()
	{
		...
		var bg:FlxSprite = new FlxSprite(0,
			0).makeGraphic(FlxG.width, FlxG.height, FlxColor.BLACK);
		bg.scrollFactor.set();
		bg.alpha = 0.4;
		add(bg);
		...
		var exitButton:FlxButton = new FlxButton(FlxG.width - 200, 50, 'Exit', function()
		{
			...
			if (controlsItems[Math.floor(curSelected)] == 'Pad-Custom' || (controlsItems[Math.floor(curSelected)] == 'Hitbox' && isExtraButton)){
				trace("real");
		...
		{
			if ((controlsItems[Math.floor(curSelected)] == 'Pad-Custom' || (controlsItems[Math.floor(curSelected)] == 'Hitbox' && isExtraButton)) && resetButton.visible) // being sure about something
			{
				MobileControls.customVirtualPad = new FlxVirtualPad((controlsItems[Math.floor(curSelected)] == 'Hitbox' && isExtraButton) ? NONE : RIGHT_FULL, DODGE);
		...
		dodgePosition = new FlxText(10, FlxG.height - 24, 0, '', 16);
		dodgePosition.setFormat(Assets.getFont('assets/mobile/menu/vcr.ttf').fontName, 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK,
			true);
		dodgePosition.borderSize = 3;
		dodgePosition.borderQuality = 1;
		add(dodgePosition);
			...
			if (controlsItems[Math.floor(curSelected)] == 'Pad-Custom'|| (controlsItems[Math.floor(curSelected)] == 'Hitbox' && isExtraButton))
			{
				...
				else
				{
					if (virtualPad.buttonUp.justPressed)
						...
						moveButton(touch, virtualPad.buttonD);
				}
			}
		}

		if (virtualPad != null && ((controlsItems[Math.floor(curSelected)] == 'Pad-Custom') || (controlsItems[Math.floor(curSelected)] == 'Hitbox' && isExtraButton)))
		{
			...
			if (virtualPad.buttonD != null)
				dodgePosition.text = 'Button Dodge X:' + virtualPad.buttonD.x + ' Y:' + virtualPad.buttonD.y;
		}
	}
	...
	private function changeSelection(change:Int = 0):Void
	{
		...
		resetButton.visible = (daChoice == 'Pad-Custom' || daChoice == 'Hitbox' && isExtraButton);
		...
		dodgePosition.visible = (daChoice == 'Pad-Custom' || daChoice == 'Hitbox' && isExtraButton);
	}
	...
	private function reloadMobileControls(daChoice:String):Void
	{
		...
		switch (daChoice)
		{
			case 'Pad-Right':
				removeControls();
				virtualPad = new FlxVirtualPad(RIGHT_FULL, DODGE);
				add(virtualPad);
			case 'Pad-Left':
				removeControls();
				virtualPad = new FlxVirtualPad(LEFT_FULL, DODGE);
				add(virtualPad);
			case 'Pad-Custom':
				removeControls();
			...
			case 'Pad-Duo':
				removeControls();
				virtualPad = new FlxVirtualPad(BOTH_FULL, DODGE);
				add(virtualPad);
			case 'Hitbox':
				removeControls();
				hitbox = new FlxHitbox(3, Std.int(FlxG.width / 4), FlxG.height, [0xFF00FF, 0x00FFFF, 0x00FF00, 0xFF0000], (isExtraButton) ? DEFAULT : SPACE);
				add(hitbox);
				if(isExtraButton){
				virtualPad = MobileControls.customVirtualPad;
				add(virtualPad);
		...
		if (virtualPad != null)
			virtualPad.visible = (daChoice != 'Hitbox' && daChoice != 'Keyboard' || daChoice == 'Hitbox' && isExtraButton);
