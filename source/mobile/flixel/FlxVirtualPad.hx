...
class FlxVirtualPad extends FlxSpriteGroup
{
	...
	public function new(DPad:FlxDPadMode, Action:FlxActionMode)
	{
		...
		switch (Action)
		{
			...
			case A_B_X_Y:
				...
				add(buttonA = createButton(FlxG.width - 132, FlxG.height - 135, 'a', 0xFF0000));
			case A_B_C_D:
				add(buttonA = createButton(FlxG.width - 510, FlxG.height - 135, 'a', 0x99062D));
				add(buttonB = createButton(FlxG.width - 258, FlxG.height - 135, 'b', 0xFFCB00));
				add(buttonC = createButton(FlxG.width - 384, FlxG.height - 135, 'c', 0x4A35B9));
				add(buttonD = createButton(FlxG.width - 132, FlxG.height - 135, 'd', 0xFF0000));
			case A_B_C_X_Y:
				add(buttonC = createButton(FlxG.width - 384, FlxG.height - 135, 'c', 0x44FF00));
	...
	private function createButton(X:Float, Y:Float, Graphic:String, Color:Int = 0xFFFFFF):FlxButton
	{
		...
		button.color = Color;
		button.alpha = Init.trueSettings.get('Pad Opacity') / 100;
		#if FLX_DEBUG
